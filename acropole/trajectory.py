...
from .columns import COLS_SMOOTH, COLS_KEEP, COLS_RESAMPLE, COLS_ACFT_PARAMS, COLS_PROCESS, \
    COLS_INPUT_FUEL, COLS_FUEL, COL_ACFT_ICAO_TYPE
...
class Trajectory:
    ...
    def __init__(self, sample_rate=4.0, min_duration_s=20, min_conf_ind=0.8,
                 mock_scipy=None, mock_pkg_resources=None, mock_predictor=None, mock_utils=None):
        """
    ...
    def resample(self, df, cols_keep=COLS_KEEP, cols_resample=COLS_RESAMPLE, cols_process=COLS_PROCESS):
        """
        ...
        df[col_time] = self.pd.to_datetime(df[col_plot_date], infer_datetime_format=True)
    ...
    def fuel_prediction(self, df, acft_dict, cols_fuel=COLS_FUEL, mass_kg=None, tas=None):
        """
        ...
        for col in [col_alti_std_ft, col_grnd_spd_kt, col_true_air_spd_kt]:
            df.loc[:, self.__DERIV + self.__SEP + col] = (df[col] - df[col].shift(1)). \
                                               fillna(method=self.__BFILL) / df[col_time_last_plot]
        ...
        if acft_type in acft_dict.keys():
            ...
            if params[col_conf_ind] > self.min_conf_ind:
                df.loc[:, col_estim_fuel_flow_acrpl] = self.predictor.predict_fuel_generic(input_values)
            else:
                df.loc[:, col_estim_fuel_flow_acrpl] = None

            df.loc[:, col_estim_fuel_flow_kgh] = (df[col_estim_fuel_flow_acrpl] * df[col_engine_num] *
                                                  df[col_fuel_flow_to] * self.__HOUR)
            df.loc[:, col_estim_conso_kg] = df[col_estim_fuel_flow_kgh] * df[col_time_last_plot] / self.__HOUR
        else:
            for col in COLS_ACFT_PARAMS[1:]:
                df.loc[:, col] = None
    ...
    def trajectory_process(self, df, cols_keep=COLS_KEEP, cols_resample=COLS_RESAMPLE, cols_smooth=COLS_SMOOTH,
                           mass_kg=None, tas=None, apply_smoothing=False):
        """
