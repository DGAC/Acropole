...
from .variables import DERIV, SEP, DEFAULT_MASS, HOUR, BFILL, \
    DEFAULT_MIN_CONF_IND, ACFT_ACRPL
...
def resample(df, cols_keep=COLS_KEEP, cols_resample=COLS_RESAMPLE, cols_process=COLS_PROCESS, samples_rate=4):
    """
    ...
    :param samples_rate: Sample rate in seconds as an Integer (default is 4)
    ...
    r = np.arange(start, end, samples_rate)
    res_df = pd.DataFrame()
...
def smooth(df, cols_smooth=COLS_SMOOTH, window_width=4):
    """
...
def fuel_prediction(df, acft_dict, min_conf_ind=DEFAULT_MIN_CONF_IND, cols_fuel=COLS_FUEL,
    ...
    for col in [col_alti_std_ft, col_grnd_spd_kt, col_true_air_spd_kt]:
        df.loc[:, DERIV + SEP + col] = (df[col] - df[col].shift(1)).\
                                               fillna(method=BFILL) / df[col_time_last_plot]
    ...
    if acft_type in acft_dict.keys():
        ...
        df.loc[:,  col_estim_fuel_flow_kgh] = (df[col_estim_fuel_flow_acrpl] * df[col_engine_num] *
                                               df[col_fuel_flow_to] * HOUR)
        df.loc[:,  col_estim_conso_kg] = df[col_estim_fuel_flow_kgh] * df[col_time_last_plot] / HOUR
    else:
        for col in COLS_ACFT_PARAMS[1:]:
            df.loc[:, col] = None
...
def trajectory_process(df, cols_keep=COLS_KEEP, cols_resample=COLS_RESAMPLE, cols_smooth=COLS_SMOOTH,
                       acft_dict=acft_cache(ACFT_ACRPL), mass_kg=None, tas=None, apply_smoothing=False):
    """
